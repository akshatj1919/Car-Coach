...
def read_scores():
    try:
        with open("data/quiz_results.txt", "r", encoding="utf-8") as f:
            t = f.read().strip()
...
class App(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkLabel(top, text="Car Coach", font=("Arial", 20, "bold")).pack(side="left", padx=8)
        ...
        ctk.CTkSwitch(top, text="Dark mode", variable=self.dark,
                      onvalue="On", offvalue="Off", command=self.change_mode).pack(side="right", padx=8)

        bar = ctk.CTkFrame(self)
        bar.pack(pady=6)
        ctk.CTkButton(bar, text="Car Basics", width=150, command=self.show_basics).pack(side="left", padx=5)
    ...
    def show_basics(self):
        ...
        class Basics(ctk.CTkFrame):
            def __init__(self, parent):
                ...
                ctk.CTkLabel(self, text="Car Basics", font=("Arial", 14, "bold")).pack(anchor="w", padx=8, pady=4)

                row = ctk.CTkFrame(self)
                row.pack(fill="x", padx=8, pady=4)
                self.query = ctk.StringVar()
                ctk.CTkLabel(row, text="Search:").pack(side="left")
                e = ctk.CTkEntry(row, width=250, textvariable=self.query, placeholder_text="Type...")
                ...
                self.textbox.insert("0.0", "Select a topic to see details.")
    ...
    def show_quiz(self):
        class Quiz(ctk.CTkFrame):
            def __init__(self, parent):
                super().__init__(parent)
                ctk.CTkLabel(self, text="Road Rules Quiz", font=("Arial", 14, "bold")).grid(row=0, column=0, columnspan=2, sticky="w", padx=8, pady=4)
                form = ctk.CTkFrame(self)
                form.grid(row=1, column=0, columnspan=2, sticky="w", padx=8)
                self.name = ctk.StringVar()
                self.level = ctk.StringVar(value="easy")
                ctk.CTkLabel(form, text="Name:").grid(row=0, column=0)
                ctk.CTkEntry(form, width=180, textvariable=self.name).grid(row=0, column=1, padx=6)
                ctk.CTkLabel(form, text="Level:").grid(row=1, column=0)
                ctk.CTkOptionMenu(form, values=["easy", "medium", "hard"], variable=self.level, width=90).grid(row=1, column=1, padx=6)
                ...
                self.left.grid(row=2, column=0, sticky="nsew", padx=(8,4), pady=8)
                self.right = ctk.CTkFrame(self)
                self.right.grid(row=2, column=1, sticky="nsew", padx=(4,8), pady=8)

                self.out = ctk.CTkTextbox(self.right)
                self.out.pack(fill="both", expand=True, padx=6, pady=6)
                self.out.insert("0.0", "Results here."); self.out.configure(state="disabled")

                btns = ctk.CTkFrame(self)
                btns.grid(row=3, column=0, columnspan=2, sticky="w", padx=8)
                ctk.CTkButton(btns, text="Start", command=self.start).pack(side="left", padx=4)
            ...
            def start(self):
                ...
                for i, q in enumerate(self.qs):
                    box = ctk.CTkFrame(self.left); box.pack(fill="x", pady=4)
                ...
                ctk.CTkButton(self.right, text="Submit", command=self.submit).pack(padx=6, pady=6, anchor="e")
            ...
            def submit(self):
                ...
                save_score(self.name.get() or "Anon", score, len(self.qs))
    ...
    def show_loan(self):
        class Loan(ctk.CTkFrame):
            def __init__(self, parent):
                super().__init__(parent)
                ctk.CTkLabel(self, text="Loan Calculator", font=("Arial", 14, "bold")).pack(anchor="w", padx=8, pady=4)

                def field(lbl):
                    f = ctk.CTkFrame(self); f.pack(anchor="w", padx=8, pady=2)
                ...
                self.out = ctk.CTkTextbox(self, height=100); self.out.pack(fill="x", padx=8, pady=6)
                ctk.CTkButton(self, text="Calculate", command=self.calc).pack(anchor="w", padx=8, pady=6)
